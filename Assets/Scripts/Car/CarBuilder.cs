...
public class CarBuilder
{
    ...
    {
        return BuildCar(spawnPoint.position, spawnPoint.rotation, InputManager.instance);
    }

    public RaceCar BuildCar(Vector3 position, Quaternion rotation, ICarInput input)
    {
        var prefabs = new CarPrefabLoadModel(m_carPrefabName, m_carWheelName);
        var prefabLoader = new CarPrefabLoadController(prefabs);

        var carCore = Object.Instantiate(prefabLoader.carCore, position, rotation);
        ...
        m_raceCar.Initialize(m_carDesc, rb, wheelRoots, wheelVisuals, input);
